...
\captionof{figure}{The code in \Cref{lst:ggpcp-api} describes the \hh{data handling for} this parallel coordinate plot with both categorical and continuous data shown on vertical axes.\label{fig:api-fig-print}}
\end{minipage}
...
From the user's perspective this data reshaping is simply \hh{a ??} data selection: the data wrangling takes place behind the scenes in \code{pcp\_select(data, ...)}, which selects the variables to be included in the parallel coordinate plot.
...
\item \code{pcp\_level}: character variable containing the factor levels of selected data variables. In case of numeric variables, the data values are stored (in textual form). The ordering of factor variables will be discussed below but \hh{it} is implemented using this added variable.
\end{itemize}
...
<<scale, fig.cap="Two scaling methods showing fatty acid compositions of  olive oils from different regions in Italy, areas within each region are colored using similar hues within region (green for Northern Italy, purple for Sardinia, and tans for Southern Italy).\\hh{robust scale: median to 0.5, see docu. Looks right but might be cryptic. check units XXX} The two scaling methods roughly allow the same conclusions.", fig.height = 8, fig.width = 8>>=
...
%\hh{Values outside this range, as can be seen for several acid measurements in \autoref{fig:scale} indicate a variability in the measurements  larger than that of a normal distribution. }
...
While hierarchical sorting requires additional computations relative to the jittering or equally spaced solutions in \Cref{fig:alignment}, this extra processing functions \hh{XXX I mistook this 'functions' as a noun in my first read, probably triggered by the hierarchical sorting. How about 'works' or 'serves as'?} as ``external cognition" \citep{scaifeExternalCognitionHow1996} - the additional computer time reduces the cognitive load required to untangle the data as displayed in the chart.
