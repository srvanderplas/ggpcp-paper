...
cols <-  c(oranges[3], purples[3], greens[3])
...
penguins %>%
  ...
  scale_color_manual("Species", values = c(oranges[3], purples[3], greens[3]))
@
...
<<alignment, fig.height = 3.5, fig.align="center", fig.cap="\\svp{Using} 12 randomly sampled penguins from the Palmer penguin data, we show four different approaches of dealing with categorical variables: the panel on the left shows the typical net of lines resulting from categorical variables in regular parallel coordinate plots. In the other three panels, ties in categorical levels are broken using different approaches (from left to right): jittering, equi-spaced line segments and ordered equi-spaced line segments are shown. ">>=
set.seed(20200924)
...
p1 <- dfnew %>%
  ...
  theme_bw() +
  scale_color_manual(values = c(oranges[3], purples[3], greens[3])) +
  xlab("") + ylab("") +
...
p3 <- dfnew %>%
  ...
  theme_bw() + ylab("") +
  scale_color_manual(values = c(oranges[3], purples[3], greens[3])) + xlab("") +
