...
ggvars <- var_id %>%
  ggplot(aes(x = var_y, y = var_scores, colour = `round(Average)`)) +
  geom_point(alpha = 0.25) + xlab("Var(pcp_y)") + ylab("Var(scores)") +
  scale_color_manual(values=c( "#045a8d", "#74a9cf",  "#fdbe85", "#f03b20", "#bd0026")) +
  theme(legend.position = "none") +
  geom_point(data = var_id %>% filter(var_y > 0.03))
#  scale_x_sqrt() +
#  scale_y_sqrt()
