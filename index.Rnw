...
\captionof{figure}{The code in \Cref{lst:ggpcp-api} generates this parallel coordinate plot with both categorical and continuous data shown on vertical axes.}
\end{minipage}
<<api-fig-print, echo = F, include = F, fig.width = 8, fig.height = 3>>=
pcp +
...
\svp{The specific method used to arrange observations within levels has a large visual impact; we will first explain the ordering approach and associated data management, and then we will assess the implications for construction of generalized PCPs.}

\svp{Tie-breaking (and the associated observation ordering)} is implemented in the \pkg{ggpcp} function \code{pcp\_arrange(data, method, space)}.
