...
Obviously, we can assess `flat-ness' of the poly-lines \svp{numerically} as the variance of the calculated variable \code{pcp\_y}. }
...
In order to maintain a persistent ordering over different values of $k$ we reorder the cluster \svp{labels} by the value of \code{body\_mass\_g}. This helps us to compare between $k$ and $k+1$ clusters.
%
