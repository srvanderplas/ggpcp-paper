...
pcp <- penguins %>%                       # data management:
  ...
  pcp_select(3:6, sex, species) %>%       #   variable selection, see section 3.1
  ...
  ggplot(aes_pcp()) +                     # plotting the chart:
    ...
    geom_pcp(aes(colour = species), alpha = 0.8, overplot="none") +         #   parallel coordinate line segments
...
\hh{XXX We still need a line of what we see from the plot.}
