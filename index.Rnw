...
Modifications of parallel coordinate plots have been specifically developed to deal with categorical data: parallel set plots \citep{Kosara:2006}, \svp{Sankey diagrams \citep{pople01,schmidt08}, Alluvial plots \citep{brunson20}}, Hammock plots \citep{Schonlau:2003}, and common angle plots \citep{Hofmann:2013}; unfortunately, these solutions do not accommodate quantitative variables.
Instead, they are intended for use with tabular data and show bands of observations from one categorical variable to the next. Hammock plots and common angle plots \svp{additionally} mitigate effects of the sine-illusion \citep{sine, susan:2015} on parallel sets plots.
...
Reducing the number of line crossings at \emph{non-axis} points simplifies the plot, reducing the overall cognitive load required to "untangle" (literally and metaphorically) the individual observations \svp{and leveraging the Gestalt principle of common fate}.
