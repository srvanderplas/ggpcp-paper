...
\svp{Users can transform individual variables, reordering factors or reversing an axis, using a mutate statement before variable selection, as shown in \autoref{lst:reverse-axis}, which generates \autoref{fig:reordering-variables-axis1}. Univariate transformations like these may be useful to reduce the overall visual complexity of a parallel coordinate plot by reducing the number of negatively correlated axes and crossing lines which are hard to follow.}
...
\au{Instead of coloring by species, we can color by sex and compare the distributions of the measurements for males and females, as is done in \autoref{fig:penguins}.  This also helps to investigate which sex might be the few penguins whose sex is not given \citep{renner1998}.
}
