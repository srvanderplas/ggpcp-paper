...
<<carcinoma2, fig.cap="Closer look at pathologists' evaluations on a more detailed scale from 1 (Negative) to 5 (Invasive Carcinoma). Average scores greater than 2 are mapped to color to distinguish likely carcinoma cases from negative cases.", fig.height = 4, fig.width = 7, out.width='0.8\\textwidth'>>=
...
Carcinoma %>%
  ...
  ggplot(aes_pcp()) +
    ...
    theme_bw() +
    scale_x_discrete(expand = expansion(add=c(0.2, 0.0))) +
    xlab("Pathologist") + ylab("Carcinoma score") +
#    theme(axis.text.y=element_blank(),
#          axis.ticks.y=element_blank())
    theme(legend.position="none") +
    scale_y_continuous(position = "right", breaks = 0:4/4, label=1:5) +
  scale_colour_manual(values=c("#3a99b8", "#E6A421"))
@
