...
\author{%
\textsc{Susan Vander Plas}$^1$\thanks{Corresponding author: \href{mailto:xxx}{xxx}}\hspace{2em}
\textsc{Yawei Ge}$^2$\hspace{2em}
\textsc{Antony Unwin}$^3$\hspace{2em}
\textsc{Heike Hofmann}$^2$\\[1ex] % Your name
{\footnotesize
$^1$ University of Nebraska Lincoln;
$^2$ Iowa State University;
$^3$ Augsburg University, Germany
} %
}
...
In addition, \pkg{ggpcp} \svp{uses} the full \svp{grammar of graphics} philosophy instead of
%using
highly specific wrapper functions, allowing users to focus on the data, rather than the names of various parameters used for customization.
...
\svp{The consequences of the choice to base \pkg{ggpcp} on the grammar of graphics framework, and the separation of the data wrangling and plotting, are discussed in \Cref{sec-api-consequences}.}
...
Similarly to previous implementations of parallel coordinate plots which attempted to accommodate categorical variables, we \svp{initially} treat factor variables as variables with labels and an associated (numerical) ordering of those labels.
...
Where \pkg{ggpcp} differs from previous implementations of parallel coordinate plots is in the assignment of numerical values to individual observations within a factor level. This
\svp{process} % ordering
...
\code{pcp\_scale(data, method)} scales the values on each axis and determines the relative relationship of the axes to each other. The \code{method} argument is a character string specifying the method to be used when transforming the values of each variable onto a common y axis. \svp{The} default, \code{uniminmax}, univariately scales each variable onto a range of [0,1] with the minimum at 0 and the maximum at 1. \code{globalminmax} maps the values across all axes onto a an interval of [0,1]. This method should only be used if the values across all variables are comparable.
...
\au{XXX Is this really what is done?  Why a confidence interval?  That does not match Figure 4 XXX}
Values outside this range, \svp{as in the top plot in} \Cref{fig:scale}, indicate a variability in the measurements  larger than that of a normal distribution, as can be seen for several acid measurements.
% Should "top plot in Figure 4" be in brackets at the end of the sentence?
\hh{For convenience, text labels instead of numeric values are shown along the $y$ axis in \Cref{fig:scale}.} \au{XXX Is this sentence needed?  Numeric values would be incomprehensible (and different for each axis). XXX}


\Cref{fig:scale} shows  two of the scaling methods applied to the olive oil data  \citep{forina, tourr}: Measurements of fatty acids in 572 olive oils from three different regions in Italy are visualized as parallel coordinate plots. Similar to the findings by \citet{ggobi}, we see that eicosenoic acid is only found in increased quantities in olive oils from Southern Italy. Quantities of oleic and linoleic acids allow a separation between olive oils from Sardinia and Northern Italy. Both scaling methods enable us to find these conclusions. While uniminmax scaling uses the space allotted to the chart most efficiently, the robust normalization method emphasizes the heavy tails and skewness of some of the measurements, such as the percentages of stearic and arachidic fatty acids. \svp{XXX I almost prefer this paragraph first in this section, thoughts? XXX}
...
\svp{As discussed previously, o}ne of the primary advantages of \pkg{ggpcp} over previous parallel coordinate plot software packages is that \pkg{ggpcp} handles categorical and continuous data in a way that allows users to trace a single observation through the projective space.
...
While \textit{automatic} sorting of factor levels is computationally difficult and statistically undesirable given that many factors have some implicit or explicit ordering that should not be automatically optimized, \svp{\textit{manually}} reordering factors can reduce the number of line crossings to produce a simpler and more comprehensible PCP.
...
\svp{While \pkg{ggpcp} allows us to follow a single observation through a plot, a}s the number of observations increases, this becomes
\svp{more difficult} %less feasible
\svp{due to overplotting} %because of overplotting of line segments
, particularly for larger data sets. As more observations and line segments are drawn, more lines cross each other, increasing the effort required to follow a poly-line from one side of the plot to the other.
...
<<carcinoma3, dependson='carcinoma2', fig.cap="Highlighting lines with high variability. XXX Need more of a description here! What are the benefits of looking at the variability of pcpy compared to the scores?", fig.height = 6, fig.width = 8>>=
...
\section{\pkg{ggpcp} API Design}\label{sec-api-consequences}
...
\svp{Using the \pkg{ggplot2} grammar of graphics API as a foundation has several benefits: users can make use of small multiple plots using familiar functions like \texttt{facet\_wrap} and \texttt{facet\_grid}, as in \Cref{fig:penguins-variable-order}.}
...
In addition, \svp{designing} \pkg{ggpcp} \svp{using the} \pkg{ggplot2} \svp{framework} expands the functionality available to users without much additional code, thanks to other packages (\pkg{plotly}~\citep{plotly-book}, \pkg{gridSVG}~\citep{gridsvg}) which extend \pkg{ggplot2} to create interactive graphics for the web.
...
\au{such as 'showPoints' for drawing points or 'boxplot' for drawing a boxplot.}
\au{These parameters become unnecessary} in the ggpcp implementation because of the direct availability of the ggplot2 layer system.
Should there be a need for boxplots \svp{on top of the default PCPs}, a simple call to `geom\_boxplot` will draw them, and depending on whether this call happens before or after a call to `geom\_pcp`, the boxplots end up \svp{behind} %in the back
or \svp{in front of} %on top of
the lines.}
...
\svp{The use of data wrangling functions outside of the plotting apparatus allows us to format the data in a consistent way (and complete that operation once). This is somewhat analogous to the way the \pkg{sf} package handles plotting based on different geometries: by customizing the format of the data that is passed into the ggplot extensions, we can write plotting functions that conform to the expected use of ggplot2 layers that use data with a somewhat different underlying structure.}
