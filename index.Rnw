...
\svp{As $k$-means uses random cluster seeds, a different starting seed value will change the results of each set of clusters. After repeating this exercise with several different seeds, it is clear that gender and species are both important variables, but which species and gender split off first changes based on the seed value.}
