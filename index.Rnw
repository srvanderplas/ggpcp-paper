...
Values outside this range, \svp{as in} \autoref{fig:scale}, indicate a variability in the measurements  larger than that of a normal distribution, as can be seen for several acid measurements. \hh{For convenience,   text labels instead of numeric values are shown along the $y$ axis in \autoref{fig:scale}. }
...
All of these parameters become unnecessary in the ggpcp implementation because of the direct availability of the ggplot2 layer system. So, should there be a need for boxplots \svp{on top of the default PCPs}, a simple call to `geom\_boxplot` will draw them, and depending on whether this call happens before or after a call to `geom\_pcp`, the boxplots end up
\svp{behind} %in the back
or \svp{in front of} %on top of
the lines. Similarly, we can deal with points, or separately drawn lines on top (e.g. to show a median cluster point).}
