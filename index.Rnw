...
library(knitr)
knitr::opts_chunk$set(fig.path="figures/", echo=FALSE, out.width = '\\linewidth', warning=FALSE, message=FALSE, cache = TRUE, dpi = 300)
...
This approach is similar to others \citep{graham_kennedy03, holten_vanwijk10} that support following an observation through a sequence of ties on the vertical axes. Instead of modifying the line slope and curvature, we apply the same transformation to each axis, ensuring that lines do not intersect.
