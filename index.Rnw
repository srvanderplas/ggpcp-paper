...
\if0\blind
{
  ...
  \author{Susan Vander Plas\thanks{
...
While parallel coordinate plots are a powerful tool, using categorical variables alongside quantitative variables \svp{in PCPs} is a great challenge.
...
\Cref{sec:examples} provides \svp{several} examples which highlight \svp{different} uses of generalized PCPs in exploratory settings.
...
make it difficult to follow a single observation across the chart. \pkg{ggpcp} alleviates this difficulty with two \svp{innovations}: careful treatment of categorical variables to prevent line intersections at vertical axes, which maintains the visual ability to follow individual cases  across the chart, and methods for ordering observations within categorical variables to reduce the amount of visual clutter.
Together, these features allow for easier perception of lines in generalized parallel coordinate plots: by reducing the number of intersecting lines at pivot points along the vertical axes \svp{through case ordering}, we allow our brains to leverage the gestalt principle of good continuation to follow one line across the plot.
...
\hh{Both scaling methods are implemented as part of } \code{pcp\_scale}.
...
<<carcinoma3, dependson='carcinoma2', fig.cap="Highlighting scans with high variability in line segments. Var(pcp\\_y) captures line variability.  XXX Need more of a description here! What are the benefits of looking at the variability of pcpy compared to the scores?", fig.height = 6, fig.width = 8>>=
...
\svp{Using the \pkg{ggplot2} grammar of graphics API as a foundation has several benefits: \hh{users of ggplot2 and the  tidyverse  come with a general understanding of how an API is set up and **should** work.
...
In addition, \svp{designing} \pkg{ggpcp} \svp{using the} \pkg{ggplot2} \svp{framework} expands the functionality available to users without much additional code, thanks to other packages, such as \pkg{plotly}~\citep{plotly-book} and \pkg{gridSVG}~\citep{gridsvg}, which extend \pkg{ggplot2} to create interactive graphics for the web.
...
\svp{While we make use of the general \pkg{ggplot2} API for plotting the data, \pkg{ggpcp} makes the additional decision to separate the data wrangling from the plotting.}
\svp{This is a slight deviation from the \pkg{ggplot2} extension approach, as} \hh{data summaries and modifications \svp{in ggplot2} are implemented in \textit{statistics} functions}. \svp{These functions}, \hh{named `stat\_xxx`, where `xxx` is usually the name of the corresponding geom to capture the close relationship between the `geom\_xxx` and the `stat\_xxx` functions.
For example, `stat\_boxplot` \svp{calculates} the summaries necessary for \svp{drawing a boxplot, such as the mean, quartiles, and IQR}.
...
\au{Splitting data wrangling and plotting provides several benefits.}
...
\au{modularizing and exposing the data wrangling pipeline into individual steps creates a better conceptual framework and gives users more flexibility to make changes: users can interact with and modify intermediate results at each step.}
\item{\bf Reducing the \svp{function} clutter: }\hh{
...
\au{Modularizing the code for data wrangling and visualizing means that parameters for parallel coordinate plots can be placed directly in their relevant functions. This helps with interpreting parameters as well as removing the need for calling the same parameters in multiple layers to ensure that all layers are based on the same data.}
\end{enumerate}

\svp{The use of data wrangling functions outside of the plotting apparatus allows us to format the data in a consistent way (and execute that operation only once). This is somewhat analogous to the way the \pkg{sf}  package \citep{sf} handles plotting based on different geometries: by customizing the format of the data that is passed into the ggplot extensions, we can write plotting functions that conform to the expected use of ggplot2 layers that use data with a  different underlying structure.}
...
\hh{This manuscript has been created in the reproducible Rweave format \citep{knitr1, knitr2} in R \citep{R} using the RStudio IDE (version Spotted Wakerobin). All original files, data, and code can be found at \url{https://github.com/srvanderplas/ggpcp-paper}. The `ggpcp` package  is available from CRAN (XXX will be).
We would like to thank all the contributors to open software, in particular, the authors behind \svp{the}  `tidyverse` packages \citep{tidyverse}. We acknowledge that a lot of this work is only possible with the help of unpaid volunteers \svp{and developers}.
}
