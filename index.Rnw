...
<<clusters, fig.height = 4, fig.width = 8, fig.cap="Overview of all variables involved in the clustering. ", out.width='\\textwidth'>>=
wide <- wide %>% mutate(elevation = `elevation-Jan`)

means <- wide %>%
  pcp_select(53:59) %>%
  select(starts_with("pcp_"), starts_with("cl")) %>%
  select(-starts_with("cloud")) %>%
  pivot_longer(cols = starts_with("cl"), names_to="cluster", values_to = "levels") %>%
  group_by(pcp_x, cluster, levels) %>%
  summarize(value = mean(pcp_y)) %>%
  pivot_wider(names_from = "cluster", values_from="levels") %>%
  pivot_wider(names_from = "pcp_x", values_from = "value")
