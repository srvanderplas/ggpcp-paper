...
cols <-  c(oranges[2], purples[2], greens[2])
...
\au{While parallel coordinate plots are a powerful tool}
\svp{ using categorical variables alongside quantitative variables is a great challenge.} In current solutions, levels of categorical variables are transformed to numbers and variables are then used as if they were numeric. This introduces ties into the data, and the resulting parallel coordinate plot becomes uninformative, as it only shows a mesh of lines from each level of one variable to each level of the next variable.
Some versions of parallel coordinate plots have been specifically developed to deal with categorical data: parallel set plots \citep{Kosara:2006}, Hammock plots \citep{Schonlau:2003}, and common angle plots \citep{Hofmann:2013}, but these solutions do not accommodate quantitative variables.
...
%
% \begin{itemize}
% \item The function \code{parcoord} in the \pkg{MASS} package \citep{MASS} makes use of the base plot system of R to draw parallel coordinate plots.
% \item The function \code{cpcp} in package \pkg{iplots} implemented the parallel coordinate plot \citep{cpcp}, \svp{but the function is not available in modern versions of the package. It is provided as an internal package function in the \pkg{extracat} package for posterity's sake.}.
% \item The \code{scpcp} function of the \pkg{extracat} package has been previously discussed. It allows use of categorical and quantitative variables, but treats categorical variables as numeric.
% \item The \pkg{GGally}\citep{ggally} package implements parallel coordinate plots using the grammar of graphics \citep{wilkinson:1999} framework of \pkg{ggplot2}\citep{ggplot2} with the \code{ggparcoord} function. It does not leverage ggplot2's extension framework; as a result, the \code{ggparcoord} function has a lot of parameters and cannot leverage the layered framework of ggplot2 natively.
% \item The \pkg{ggparallel}\citep{ggparallel} package provides an implementation of hammock and common-angle plots, but does not accommodate quantitative data. This package also leverages \pkg{ggplot2} under the hood but cannot be extended easily.
% \end{itemize}
...
<<sketch, warning=FALSE, results='asis', fig.width = 9, fig.height = 3, fig.cap="Sketch of a parallel coordinate plot of two observations in four dimensions. Each dimension is shown as a vertical axis, observations are connected by polylines from one axis to the next.\\au{Two penguins from the Palmer Penguin data set (see section 5) were sampled for this example.}">>=
...
\au{An important motivation for \hh{the `ggpcp`} package is that other implementations of parallel coordinate plots for categorical variables }
...
\svp{make it difficult to follow a single observation across the chart. `ggpcp` alleviates this difficulty with two new developments: careful treatment of categorical variables to prevent line intersections at vertical axes, which maintains the visual ability to follow \au{individual cases}  across the chart, and methods for ordering observations within categorical variables to reduce the amount of visual clutter. Together, these features allow for easier perception of lines in generalized parallel coordinate plots: by reducing the number of intersecting lines at pivot points along the vertical axes, we allow our brains to leverage the gestalt principle of good continuation to follow one line across the plot. Reducing the number of line crossings at non-axis points simplifies the plot, reducing the overall cognitive load required to "untangle" (literally and metaphorically) the individual observations.}

% \au{XXX Gestalt motivation of what makes this package unique - ability to follow a single data point through the parallel coordinate plot.XXX}
...
pcp <- penguins %>%                       # data management:
  pcp_select(species, 3:6, sex) %>%       #   variable selection, see section 3.1
...
@
\end{minipage}
<<echo=FALSE, eval=FALSE, results = 'hide', error=FALSE, warning=FALSE, cache=FALSE>>=
library(GGally)
data(flea)
@

% \svp{XXX Make the code into a subfigure and put alongside figure 1. Switch to penguins data, select 2 penguins from the set. XXX} % I no longer really understand why we'd have this with Fig 1, since it's entirely different?


\svp{In addition, \pkg{ggpcp} leverages}  the full ggplot2 philosophy instead of using highly specific wrapper functions, \svp{allowing} users to focus on the data, rather than the names of various parameters used for customization.
...
In addition, basing \pkg{ggpcp} on \pkg{ggplot2} expands the functionality available to users without much additional code, thanks to other packages such as \pkg{plotly}~\citep{plotly-book}  which leverage \pkg{ggplot2} to create interactive graphics for the web
 and \pkg{gridSVG}~\citep{gridsvg} \hh{which exposes vectorized graphics to allow programmable changes, such as elementary interactions with graphical elements}.
...
\svp{One of the ideas} behind this re-implementation of parallel coordinate plots is to expose parallel coordinate plots at a functional level. Rather than using a single function with parameters controlling every aspect, we separate the data management from the visual rendering.
...
\citep{jss.tidy, hadley.tidy}, we get to a version of the dataset in which we achieve a one-to-one mapping to a now discrete $x$ axis consisting of the (names of the) original data variables.
...
From the user's perspective this data reshaping is purely a form of a data selection, while the data wrangling is going on behind the scenes in \code{pcp\_select}:

\code{pcp\_select(data, ...)} selects the variables to be included in the parallel coordinate plot.
...
As a consequence of these design decisions, users have several ways of performing different tasks within the flow of generating data for a parallel coordinate plot. For instance, users can reorder variables using \code{pcp\_select} or after variable selection using the \code{pcp\_x} variable.
...
\svp{Users can transform individual variables, reordering factors or reversing an axis, using a mutate statement bfeore variable selection, as shown in \autoref{lst:reverse-axis}, which generates \autoref{fig:reordering-variables-axis1}. Univariate transformations like these may be useful to reduce the overall visual complexity of a parallel coordinate plot by reducing the number of anticorrelated axes in sequence.}


<<reordering-variables-axis1-flea, fig.width = 8, fig.height = 4, eval = F>>=
flea %>%
  mutate(
    species_inv = factor(species, levels = rev(levels(.$species))),
    head_log = log(head)) %>%
  pcp_select(species, 2:4, head_log, 5:7, species_inv) %>%
  pcp_scale(method="uniminmax") %>%
  pcp_arrange() %>%
  ggplot(aes_pcp()) +
    geom_pcp_axes() +
...
\begin{minipage}[b]{.94\linewidth}
\captionof{lstlisting}{Mutations before the \code{pcp\_select} stage can be used to reorder factors or transform numerical variables, as shown in \autoref{fig:reordering-variables-axis1}.}\label{lst:reverse-axis}
<<reordering-variables-axis1-code, fig.width = 8, fig.height = 4,  echo = TRUE, reset = TRUE,  collapse = T, fig.show='hide'>>=
penguins %>%
  mutate(
    species_inv = factor(species, levels = rev(levels(.$species))),
    neg_bill_depth_mm = -bill_depth_mm) %>%
  pcp_select(species, neg_bill_depth_mm, 3, 6, 5, species_inv) %>%
  pcp_scale(method="uniminmax") %>%
  pcp_arrange() %>%
  ggplot(aes_pcp()) +
    geom_pcp_axes() +
    geom_pcp(aes(colour = species), alpha = 0.6) +
    geom_pcp_labels()
@
\end{minipage}

<<reordering-variables-axis1, fig.width = 8, fig.height = 4, include = T, echo = F, fig.cap = "The left-most axis shows species, where the right-most axis shows the same variable with categories reordered; similarly, \\texttt{neg\\_bill\\_depth\\_mm} shows the bill depth, multiplied by -1 to invert the order of data points.">>=
penguins_pcp2 <- penguins %>%
  mutate(
    species_inv = factor(species, levels = rev(levels(.$species))),
    neg_bill_depth_mm = -bill_depth_mm) %>%
  pcp_select(species, neg_bill_depth_mm, 3, 6, 5, species_inv) %>%
...
penguins_pcp2 %>%
  ggplot(aes_pcp()) +
    ...
    geom_pcp(aes(colour = species), alpha = 0.6,
             overplot="none") +
  xlab("") + ylab("") + scale_x_discrete(expand = c(0.05, 0)) +
...
\au{Ordered factors are plotted from the lowest level upwards.  If a factor legend is included, it will need to be reversed to match this order.  Use \code{guides(color = guide\_legend(reverse=TRUE))} as shown in the example in \autoref{sec:carcinoma}.}
<<colorscheme>>=
#####  Color Palette by Paletton.com
...
\au{Assigning all cases of the same level to the same point on the axis of a categorical variable makes it impossible to distinguish them.  The approach of `ggpcp` is to draw boxes proportional to the numbers of cases in each level and to order the cases within their boxes so that visual clutter is minimised and individual cases can be followed.}
\hh{This idea is implemented in the `ggpcp` function \code{pcp\_arrange(data, method, space)}.}
\au{The two implemented methods are \code{"from-left"} and \code{"from-right"}, meaning that ties are broken using a hierarchical ordering determined by variables’ values from the left or the right, respectively. The parameter \code{space} specifies the amount of the $y$ axis to use for space between levels of categorical variables. By default, 5\% of the axis is used for spacing.}
...
One of the primary advantages of the generalized approach to dealing with categorical variables is the ability to follow a single observation throughout the plot. As the number of observations increases, this becomes less feasible because of overplotting of line segments, particularly for larger data sets. As more observations and line segments are drawn, more lines cross each other, increasing the effort required to follow a poly-line from one side of the plot to the other. %\hh{XXX the order in which crossing lines are drawn is only visible when the lines have different colors.}
\au{Coloring by groups improves the readibility of plots.  However, the order of drawing the cases may affect what can be seen due to overplotting.}
\au{As a countermeasure, the order in which line segments are plotted should be carefully chosen. The parameter \code{overplot} offers the option "small-on-top" where groups are plotted in order of size from largest to smallest so that the smallest is on top.  The default setting "none" is very flexible, but requires the user to specify the order they want before plotting--or just accept the current order of the dataset. }
...
% %
% \end{enumerate}
\hh{XXX additional counter-measures: use $\alpha$-blending}
...
\au{The top of} \autoref{fig:ordering} shows a  generalized parallel coordinate plot of the Palmer penguins data \citep{penguins}.
\au{The numeric data consists of body measurements of three species of penguins: bill length, bill depth, flipper length, body weight.}
 What can be seen is that Adelie penguins generally have smaller bill lengths than the other two species, while Gentoo penguins can be distinguished by their relatively large flipper lengths.
\au{The bottom of} \autoref{fig:ordering} shows the effect of re-ordering the levels of both the `species` and the `island` variables in the generalized parallel coordinate plots. \svp{This re-ordering of factor levels has the effect of emphasizing that Gentoo penguins and Chinstrap penguins are each found on only one island, while Adelie penguins are found on all three islands.}

\hh{Only after levels of `island` and `species` are re-ordered, we can see}
\au{%It further clarifies
...
%\au{study ID is what? ordering in second plot allows us to see that about the same amount of datat from penguins in each species was collected. }
...
\au{As already explained, ggpcp is part of the ggplot2 family, so faceting is easy.  This has been done in \autoref{fig:ordering} to show that the patterns are similar for both males and females.}

\au{figure 6 and 7 look quite similar, but are quite different - visual emphasis? Maybe use colors[1] and colors [3]?}

In addition to the ordering of factor levels, it is also important to carefully order the variables on the $x$-axis, as shown in \autoref{fig:penguins-variable-order}, where the variables have been re-ordered
to allow the viewer \au{identify which} body measurements distinguish the species. \au{Besides the re-ordering, the axis for bill depth has been reversed. } \au{Both changes help to separate the species.} \hh{ Gentoo penguins have the lowest bill depth, while generally having the longest flippers and being heaviest. Reversing the axis for bill depths, aligns the smallest bill depths with the longest flippers, thereby moving Gentoo penguins closer together as a group. }
...
\hh{Note that we are using facetting by penguins' sex} in \autoref{fig:penguins-variable-order}. \hh{While all of the results are the same for the two sexes, any variability of body measures due to sex is removed from the plot by facetting. This makes the results stand out more.}

%The plot does not show anything about the differences between male and female penguins.  Given the greater size of the males \au{we don't know that yet} in general it may be better to split by male and female, ignoring the missing values.  The results are the same for both sexes and stand out more.
Interestingly,  some potential outliers that were not visible previously, now become visible. \au{Note for example the two Gentoo males with particularly short flippers.}
...
#  pcp_select(studyName, species, island, `-CulmenD`, FlipperL, BodyMass, CulmenL) %>%
  ...
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(),
        legend.position="none") +
  scale_colour_manual(values=c(purples[c(1,3)], oranges[c(1,3)], greens[c(1,3)])) +
  facet_grid(Sex~.)
@
...
%
<<penguins, fig.cap="Generalized Parallel Coordinate Plot of the Palmer penguins data with sex of penguin mapped to color. Dark lines represent penguins for which sex could not be determined.  We see that researchers were able to sex all of the Chinstrap penguins. Note that species is included twice (with different order of the levels). \\hh{XXX reason for that here or in the text?}", fig.height=4, fig.width=8>>=
 penguins <- penguins %>%
   mutate(
...
gg_p1 <- penguins %>%
  filter(species != "Chinstrap", !is.na(sex)) %>%
  ggplot(aes(x = body_mass_g, y = flipper_length_mm)) +
  facet_wrap(species~sex)+
  ggdensity::geom_hdr(probs =c(0.5, 0.95))

gg_df <- gg_p1 %>% ggplot_build()
...
gg_df3 <- gg_p3 %>% ggplot_build()

gg_p1_pcp <- gg_df$data[[1]] %>%
  #  filter(nlevel==1) %>%
  select(x, y, PANEL, order) %>%
  rename(body_mass_g = x, flipper_length_mm = y) %>%
  mutate(type = "contour",
         sex = ifelse(PANEL %in% c(1,3), "female", "male"),
         species = ifelse(PANEL %in% c(1,2), "Adelie", "Gentoo"),
         bill_length_mm = NA,
         bill_depth_mm = NA
  ) %>% select(-PANEL) %>%
  rbind2(
    gg_df2$data[[1]] %>%
      #  filter(nlevel==1) %>%
      select(x, y, PANEL, order) %>%
      rename(flipper_length_mm = x, bill_length_mm = y) %>%
      mutate(type = "contour",
             sex = ifelse(PANEL %in% c(1,3), "female", "male"),
             species = ifelse(PANEL %in% c(1,2), "Adelie", "Gentoo"),
             body_mass_g = NA,
  ...
  rbind2(
    gg_df3$data[[1]] %>%
      #  filter(nlevel==1) %>%
      select(x, y, PANEL, order) %>%
      rename(bill_length_mm = x, bill_depth_mm = y) %>%
      mutate(type = "contour",
             sex = ifelse(PANEL %in% c(1,3), "female", "male"),
             species = ifelse(PANEL %in% c(1,2), "Adelie", "Gentoo"),
             body_mass_g = NA,
  ...
  rbind2(penguins %>% select(body_mass_g, flipper_length_mm, bill_length_mm,bill_depth_mm, sex, species) %>%
           mutate(type = "observed", order=NA)) %>%
  filter(species !="Chinstrap", !is.na(order)) %>%
  mutate(
    sex = ifelse(is.na(sex),"?", as.character(sex)), # make assignment more readable
    sex = factor(sex, levels = c("female", "?", "male"))
  )

gg_p1_pcp <- gg_p1_pcp %>% ungroup() %>%
  select(body_mass_g, flipper_length_mm, bill_length_mm,bill_depth_mm, order, type, species, sex) %>%
  pcp_select(body_mass_g, flipper_length_mm, bill_length_mm,bill_depth_mm) %>%
  group_by(species) %>%                                  # re-scale by species
  pcp_scale() %>%
  pcp_arrange()


contours <- gg_p1_pcp %>% filter(type=="contour") %>%
  ungroup() %>%
  group_by(sex, species, order, pcp_x) %>%
  summarize(
    pcp_y = range(pcp_y, na.rm = TRUE),
    range = c("lower", "upper")
  )

contours <- contours %>%
  pivot_wider(values_from = pcp_y, names_from = range)


gg_p1_pcp <- filter(gg_p1_pcp, type != "contour")

# gg_p1_pcp %>%
#   ggplot(aes_pcp()) +
#   geom_ribbon(aes(x = pcp_x, ymin=lower, ymax=upper, fill=sex,
#                   group = order), alpha = 0.25, data = contours) +
#   facet_grid(species~sex) +
#   geom_pcp(aes(colour = sex, colour = "?"), colour = "#8b4950dd", size = 0.9,
#            data = penguins_pcp %>% filter(sex=="?") %>% mutate(sex = "female")) +
#   geom_pcp(aes(colour = sex, colour = "?"), colour = "#8b4950dd", size = 0.9,
#            data = penguins_pcp %>% filter(sex=="?") %>% mutate(sex = "male")) +
#   theme_bw() +
#   scale_fill_manual("Sex",values=c( "#FAA404", "#008080",  "#8b4950")) +
#   scale_alpha_manual("Inter-quantile Range", values = c(0.05, 0.15)) +
#   guides(
#     colour = guide_legend(override.aes = list(alpha = 0.5)),
...
#  geom_pcp(aes(colour = sex), alpha = 0.4) +
  geom_ribbon(aes( ymin = lower, ymax=upper, group=interaction(sex,order), fill = sex, alpha=order),
              #alpha = 0.25,
              data = contours) +
  ...
  theme_bw() +
  scale_fill_manual("Sex", values=c( "#8b4950", "#FAA404", "#008080")) + # lighter yellow "#FFCD70"
  facet_grid(species~sex) +
  ...
  guides(
    ...
    alpha = guide_legend(override.aes = list(alpha =  c(0.25, 0.5)))
  ) +
...
\au{For the remaining 62.5\% of slides there was some disagreement and it is clear that this disagreement is not random.} \au{The pathologists have been ordered from left to right from the fewest number of overall carcinoma diagnoses made to the highest number.  This shows a strong level of agreement between adjacent axes.}
%When pathologists are ordered (by moving the corresponding axes) left to right from fewest number of overall carcinoma diagnoses to highest number, we see that generally for a slide more pathologists make a carcinoma diagnosis  from left to right.
...
\citet{carcinoma} (Table 1) allow us a closer look at this data.
\au{The pathologists evaluated the slides using five levels from 1 to 5}, given as:
...
\citet{agresti} \au{classified levels 1 and 2 as "no" and levels 3 to 5 as "yes".}
\autoref{fig:carcinoma2} gives an overview of this more detailed data. The different pathologists are drawn in the same order as in \autoref{fig:carcinoma}. The results for each scan are colored by the overall average score (rounded to the closest integer). Compared to the previous figure, \autoref{fig:carcinoma2} shows more variability between pathologists' evaluations, but only few scans have vastly different scores assigned to them. Pathologist C, in particular, rates two scans as negative, that all other pathologists rate as quite advanced cancer. \au{Mostly, the variability between pathologists’ scores stems from a difference in applying the scores rather than from an actual difference of opinions.} %Overall, the variability between pathologists' scores stems froma difference in applying the scores rather than an actual difference of opinions. \hh{XXX this needs to be phrased differently. What I mean is that we see a lot of agreement in the parallel lines, but the boxes are shifted from one pathologist to another.}
The similarity in evaluations is particularly striking between pathologists A, G, E, and B.

\hh{XXX very unpolished idea, but I think worthwhile exploring XXX The generalized parallel coordinate plot gives us a visual tool for assessing the similarity between evaluations by different pathologists that moves beyond a mere correspondence of scores to an analysis that is based on ranks. Tie-breaks in the ranks are featured as the $y$ coordinate.
When the poly-lines additionally are relatively flat, this means that  pathologists can either agree on the score of these scans or agree at least on the relative severity of carcinomas in the scans.
Obviously, we can assess 'flat-ness' of the poly-lines  as the variance of the calculated variable `pcp\_y`. }
...
var_id <- Carcinoma_pcp %>% group_by(pcp_id) %>%
  summarize(
    var_y = var(pcp_y)
  )
...
<<carcinoma3, fig.cap="Highlighting lines with high variability", fig.height = 6, fig.width = 8>>=
Carcinoma_pcp2 <- Carcinoma_pcp %>%
  left_join(var_id, by = "pcp_id")

var_id <- var_id %>% left_join(Carcinoma_pcp2 %>% select(pcp_id, `round(Average)`) %>% unique(), by="pcp_id")
...
hist <-
  ...
  ggplot(aes(x = var_y, fill = `round(Average)`,
             alpha = var_disc)) +
  geom_histogram(binwidth = 0.0075, colour="grey50", size=0.1) +
  scale_fill_manual("Average carcinoma score",values=c( "#045a8d", "#74a9cf",  "#fdbe85", "#f03b20",
"#bd0026")) +
  xlab("Variance of pcp_y") +
  ylab("Number of scans") +
  scale_alpha_manual("Variance in pcp_y",
                     values = c(1, 0.25)) +
...
#  scale_color_brewer(type="qual") +
  scale_color_manual(values = c(oranges[2], greens[2])) +
  theme(legend.position = "none") +
  ylab("") +
  theme(axis.title.y = NULL, axis.text.y = NULL, axis.ticks.y = NULL) +
  scale_y_continuous(labels=c("low", "", "medium", "", "high"),
...
penguins_complete %>%
  ...
  theme_bw() +
#  scale_color_brewer(type="qual") +
    scale_color_manual(values = c(oranges[1], purples[2], oranges[3], greens[2]))  +
  theme(legend.position = "none") +
  ylab("") +
  theme(axis.title.y = NULL, axis.text.y = NULL, axis.ticks.y = NULL) +
  scale_y_continuous(labels=c("low", "", "medium", "", "high"),
...
penguins_complete_pcp %>%
  ...
  theme_bw() +
    scale_color_manual(
      values = c(oranges[1], purples[1], purples[3], oranges[3], greens[2])) +
  theme(legend.position = "none") +
  ylab("") +
  theme(axis.title.y = NULL, axis.text.y = NULL, axis.ticks.y = NULL) +
  scale_y_continuous(labels=c("low", "", "medium", "", "high"),
...
penguins_complete_pcp %>%
  ...
  theme_bw() +
#  scale_color_brewer(type="qual") +
    scale_color_manual(values = c(oranges[1], purples[2], oranges[2], oranges[3], greens[1], greens[3]))  +
  theme(legend.position = "none") +
  ylab("") +
  theme(axis.title.y = NULL, axis.text.y = NULL, axis.ticks.y = NULL) +
  scale_y_continuous(labels=c("low", "", "medium", "", "high"),
...
%p1 Abstract: are a valuable tool (not useful)

%p2 an arbitrary number (not an arbitrary many number)
%p2 Projective Space or projective space? HH: let's go with lower case
%p2 as a way of getting (not to get), of identifying, of finding
%p2 While parallel coordinate plots are a powerful tool (not have some shortcomings)
%p2 to the levels of the next (not to all levels)
%p2 tabular (not tabularized)

%p3 An important motivation for our package is that other implementations of parallel coordinate plots for categorical variables (not the current first sentence)
%p3 to follow individual cases across (not to follow one line across)
%p3 It should be gridSVG and the sentence implies it helps interactive graphics, how? HH: gridSVG got pushed in between plotly and its interactivity. I tried to add something for gridSVG, but I'm not sure that my sentence is working yet.

%p4 of laying out (not to lay out)
%p4 are optional (not even optional)
%p4 the main obstacle to allowing (not the main obstacle of allowing)
%p4 two-variable associations (not two-variable comparisons)
%p4 adjacent axes (not sequential axes)
%p4 When ties cannot be separated (not When ties are not handled appropriately)
%p4 is a function between one data variable and one aesthetic (not is a function between one aesthetic and one data variable)
%p4 Four Hadley references in one place?!
%p4 is purely a form of data selection (not has purely the form of a data selection)
%p4 selects the variables (not allows a selection of variables)

%p5 ways of performing (not ways to perform)

p5-6 Why has bill_depth been inverted?  Why does species appear twice?

%p6 Last para needs a rewrite (wherever it goes).  How about:
%Ordered factors are plotted from the lowest level upwards.  If a factor legend is included, it will need to be reversed to match this order.  Use guides(color = guide_legend(reverse=TRUE)) as shown in the example in subsection 5.2.

%p8 transforming the values of each variable onto a common y axis (not transforming the values of each variable into a common y axis)
%p8 onto a range of [0,1] with the minimum at 0 and the maximum at 1 (not into a range of [0,1] with a minimum at 0 and the maximum at 1)
%p8 onto an interval of [0,1] (not into a an interval of [0,1])
%p8 applied to (not at the example of)
%p8 Forina and one other reference would do
%p8 .  Measurements of fatty (not : measurements of fatty)
%p8 allotted not alloted
%p8 Intro to §4.1  Here is one possible rewrite:
%Assigning all cases of the same level to the same point on the axis of a categorical variable makes it impossible to distinguish them.  The approach of ggpcp is to draw boxes proportional to the numbers of cases in each level and to order the cases within their boxes so that visual clutter is minimised and individual cases can be followed.  The options provided are in pcp_arrange(data, method, space).  The two implemented methods are "from-left" and "from-right", meaning that ties are broken using a hierarchical ordering determined by variables’ values from the left or the right, respectively. The parameter space specifies the amount of the ‘y‘ axis to use for space between levels of categorical variables. By default, 5% of the axis is used for spacing.
%p8 it also allows us, in the spirit of the original parallel coordinate plots, (not it also allows us in the spirit of the original parallel coordinate plots)

%p9 As the number of observations increases (not As the number of observations increase)
%p9 particularly for larger data sets (not particularly for slightly larger data sets)
%p9 Instead of "XXX the order in which crossing lines are drawn is only visible when the lines have different colors." how about
%Colouring by groups improves the reability of plots.  However, the order of drawing the cases may affect what can be seen due to overplotting.
%p9 As a countermeasure, the order in which line segments are plotted should be carefully chosen. The parameter overplot offers the option "small-on-top" where groups are plotted in order of size from largest to smallest so that the smallest is on top.  The default setting "none" is very flexible, but requires the user to specify the order they want before plotting--or just accept the current order of the dataset.  (not As a countermeasure we carefully control the order in which line segments are plotted.  Two methods to control this order are implemented and are regulated by the parameter overplot:)
%p9 Several aspects in Parallel Coordinate Plots depend on orderings: the order of variables along the x axis, the order of levels in a categorical variable, the orderings of cases within categorical variable levels, and the order in which lines are drawn. (not Several aspects in Parallel Coordinate Plots depend on orderings: order of variables along the x axis, order of levels in a categorical variable, and order in which lines are drawn.)
%p9 Orderings should therefore (a) have good defaults, and (b) be easily changeable. (not Changing orders should therefore (a) have good defaults, and (b) be easily changeable.)

%p10 Scrap my contribution in green below Fig 6
%p10 The top plot of Figure 6 shows (not Figure 6 shows)
%p10 The numeric data consists of body measurements of three species of penguins: bill length, bill depth, flipper length, body weight. (not The data consists of body measurements, such as weight, flipper length, bill length, and depth, of three species of penguins.)
%p10 while Gentoo penguins can be distinguished by their relatively large flipper lengths. (not while Gentoo penguins can be distinguished from the other two species by their relatively larger flipper lengths.)
%p10 Move the first paragraph of §5.1.1 to the end of the previous section.
%p10 The lower plot of Figure 6 shows (not Figure 6 shows)
%p10 both the ‘species‘ and the ‘island‘ variables (not both the ‘species‘ and the ‘island‘ variable)
%p10 This re-ordering of factor levels has the effect of emphasizing that Gentoo penguins and Chinstrap penguins are each found on only one island, while Adelie penguins are found on all three islands. (not This re-ordering of factor levels has the effect of emphasizing the fact that Gentoo penguins and Chinstrap penguins are each found on only one island, while Adelie penguins are found on all three islands.)
%p10 Add to the end of this new paragraph at the end of §5.1:
%It further clarifies that for each species the numbers of penguins in the three years of the study (the study ID variable) were roughly the same.

%p10 Add introductory sentence to §5.1.1:
%As already explained, ggpcp is part of the ggplot2 family, so faceting is easy.  This has been done in Figure 7 to show that the patterns are similar for both males and females.
%p10 to help the viewer identify which (not to allow the viewer to see what)

%p10-11 Besides the re-ordering, the axis for bill depth has been reversed. (not Note that besides the re-ordering, the axes for bill depth is also reversed.)
%p11 Both changes help to separate the species. (not XXX explain why this helps Again, this reversal helps with distinguishing between species:)
%p11 having the longest flippers and being heaviest (not having the longest flippers)
%p11 Scrap my XXX comment

%p11 Add intro to §5.1.2:
%Instead of colouring by species, you can colour by sex and compare the distributions of the measurements for males and females, as is done in Figure 8.  This also helps to investigate which sex might be the few penguins whose sex is not given.
%p11 Figure 8 shows that (not The lines in Figure 8 are colored by sex of penguins. What can be seen is that)

%p12 Comparing these penguins’ body measurements to those of the other penguins, we can make suggestions regarding their sex. (not Based on these penguins’ body measurements within the context of the other penguins, we can make some suggestions regarding their sex.)
%p12 The text accompanying Figure 9 will depend on what Figure 9 you draw, so I have left that.

%p13 For the remaining 62.5\% of slides there was some disagreement and it is clear that this disagreement is not random. (not For the remaining 62.5\% of slides there was some disagreement. However, we see that this disagreement is not random.)
%p13 The pathologists have been ordered from left to right from the fewest number of overall carcinoma diagnoses made to the highest number.  This shows a strong level of agreement between adjacent axes.  (not When pathologists are ordered (by moving the corresponding axes) left to right from fewest number of overall carcinoma diagnoses to highest number, we see that generally for a slide more pathologists make a carcinoma diagnosis from left to right.)
%p13 The pathologists evaluated the slides using five levels from 1 to 5 (not Pathologists evaluated the (same) scans at a five levels from 1 to 5)
%p13 Agresti and others classified levels 1 and 2 as "no" and levels 3 to 5 as "yes".  Figure 11 (not Figure 11)

%p15 Mostly, the variability between pathologists’ scores stems from a difference in applying the scores rather than from an actual difference of opinions. (not Overall, the variability between pathologists’ scores stems froma difference in applying the scores rather than an actual difference of opinions.)
%p15 k-means clustering assigns cluster labels arbitrarily based on random cluster centers.   In order to maintain a persistent ordering over different values of k we reorder the clusters by the value of body_mass_g. (not Because k-means clustering assigns cluster labels arbitrarily based on random cluster centers, in or- der to maintain a persistent ordering over different values of k we reorder the clusters by the value of body_mass_g.)
%p15 This helps us to comparing between k and k + 1 clusters. (not This helps us to compare between e.g. k and k + 1 clusters.)
%p15 Figure 12 shows the numeric measurements along with the assigned clusters, with categorical variables species and sex on the right. (not Figure 12 shows the numeric measurements along with the assigned clusters, with categorical variables sex and species on the right.)  Perhaps the sex axis should be added to the first two plots to avoid confusion.
%p15  though again there are some penguins that (not  though again there are again some penguins who)
%p15 that some of the separation into sexes (not that some of separation into sexes)

%p17 is not strong or is ambiguous (not is not strong or ambiguous)
%p17 §6
%generalized parallel coordinate plots offer a constructive solution to incorporating categorical variables in parallel coordinate plots.  In particular, they preserve the key idea of being able to follow individual cases across the plot.  As always with parallel coordinate plots the use of colour to distinguish groups, the ordering of the axes, and the ordering of drawing the lines play key roles.  With generalized parallel coordinate plots two additional kinds of ordering have a part to play, the ordering of levels of categorical variables and the ordering of cases within those levels.
%The ideas described here have been implemented in the R package ggpcp.  There are two central features.  Data management has been separated from visual rendering, and the package follows the full ggplot2 philosophy.  Both these features contribute to the power and flexibility of the software.
