...
\svp{XXX need a transition into this section - we want to reflect on the implications of the decisions to use the ggplot2 philosophy and to separate data wrangling and plotting. XXX}
...
\hh{Generally,
any data summary or modifications are implemented in \textit{statistics} functions, named `stat\_xxx`, where `xxx` is usually the name of the corresponding geom to capture the close relationship between the `geom\_xxx` and the `stat\_xxx` functions.
...
% \au{XXX Remove the stat\_pcp paragraph.  It is too technical (and negative) for what is in effect the summary of the paper.  If you want to keep it, move it to the body of the paper.  XXX}
...
\svp{XXX transition to overall strength of the grammar of graphics XXX}
...
\svp{Another strength} of \pkg{ggplot2} is its handling of small multiple plots with \code{facet\_grid} and \code{facet\_wrap}; these functions are fully supported in \pkg{ggpcp}.
...
\svp{XXX Transition to the discussion XXX}
